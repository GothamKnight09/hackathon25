...
def loan_form():
    if request.method == 'POST':
        ...
        input_data_lr = input_data_lgbm[features]

        

        
        # Apply encoder and scaler
        input_encoded_lgbm = woe_encoder.transform(input_data_lgbm)
        
        # input_encoded_lr = woe_encoder.transform(input_data_lr)
        input_scaled = scaler.transform(input_data_lr)
        ...
        predicted_loan_amount_lgbm = lgbm_loan_amount.predict(input_encoded_lgbm)[0]
        predicted_interest_rate_lgbm = lgbm_interest_rate.predict(input_encoded_lgbm)[0]
        loan_approval_lgbm = lgbm_model.predict(input_encoded_lgbm)[0]
